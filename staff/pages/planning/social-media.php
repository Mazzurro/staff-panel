...
<style>
    @font-face {
        ...
        src: url('iconfont.eot?t=1577263980856');
        /* IE9 */
        src: url('iconfont.eot?t=1577263980856#iefix') format('embedded-opentype'),
            /* IE6-IE8 */
            ...
            url('iconfont.ttf?t=1577263980856') format('truetype'),
            /* chrome, firefox, opera, Safari, Android, iOS 4.2+ */
            url('iconfont.svg?t=1577263980856#iconfont') format('svg');
        /* iOS 4.1- */
    }
...
<body>
    <div class="run-rate" id="run-rate">
        <div class="mainContent">
            <div id="searchRevenue">
                ...
                <select class="bg-account-select">
                    <option class="bg-account-select-item">Please select an account</option>
                    <input type="hidden" name="bg-account_id">
                </select>
   ...
   <script src="http://cdn.highcharts.com.cn/highcharts/css/highcharts.css"></script>
    <script>
...
<script type="text/javascript">
    $(document).ready(function() {
        $.ajax({
            ...
            success: function(data) {
                for (let item in data) {
                    $(".bg-account-select").append(`
                    		<option class="bg-account-select-item" data-id="${data[item].social_media_account_id}">[${data[item].social_media_type}]  ${data[item].social_media_account_name}</option>`);
                }
                $('.bg-account-select').on('change', function() {
                    $('.bg-account-select').find('option:selected')
                    //                  		console.log($(this)[0].selectedIndex+1)
                    $("input[name=bg-account_id]").val($('.bg-account-select').find('option:selected').data("id"));
                })
            }
        })
        ...
        $('.addAuthority').click(function() {
            let account = $("input[name=bg-account_id]").val()
            console.log(account)
            if (account == undefined || account == "") {
                addNotif("Please select an account first", 2);
            } else {
                $('#run-rate').addClass('panel-popup-active loading');
                let randAppID = paneljs.genID(5);
                $('#run-rate').append(`<div id="app-` + randAppID + `"></div>`);
        ...
        $.ajax({
            type: "POST",
            url: "http://192.168.50.90/staff/api/Socialmedia/List2",
            success: function(data) {
                console.log(data)
                for (let item in data) {
                    $('.mainContent3').append(`
		        		<div class="content">
		                	<div class="content1"><div class="timetable-data timetable-options" style="background:#96031a;color:white">${data[item].location_city},${data[item].location_country}</div></div>
			                <div class="content2">
